    ...
    for i in range(recur_layers):
        if i == 0:
            rnn = GRU(units, activation=activation, return_sequences=True, 
                 implementation=2, name='rnn'+str(i))(input_data)
...
def final_model(input_dim, filters, kernel_size, conv_stride, 
    ...
    for i in range(recur_layers):
        if i == 0:
            rnn = Bidirectional(GRU(units, activation=activation, return_sequences=True, 
                 implementation=2, name='rnn'+str(i)))(bn_cnn)
            bn_rnn = BatchNormalization(name='bn_rnn'+str(i))(rnn)
            dropout = Dropout(0.5)(bn_rnn)
        else:
            rnn = Bidirectional(GRU(units, activation=activation, return_sequences=True, 
